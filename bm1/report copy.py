...
class Report:
    ...
    def generate_sales_report(self):
        ...
        try:
            ...
            cursor.execute("SELECT orders.id, users.username, orders.total, orders.created_at, DATE(orders.created_at) as order_date FROM orders JOIN users ON orders.username = users.username")
            orders = cursor.fetchall()
            
            # Query to fetch the number of ordersand their total value per day
            cursor.execute('''SELECT DATE(created_at) as order_date, COUNT(*) as total_orders, SUM(total_price) as total_value FROM orders GROUP BY DATE(created_at)''')
            ...
            table = PrettyTable()
            table.field_names = ["Orders ID", "Username", "Total Price", "Date"]
    ...
    def generate_user_report(self):
        ...
        try:
            ...
            # TIMESTAMPDIFF(MINUTE, MIN(orders.order_time), MAX(orders.order_time)) as time_spent